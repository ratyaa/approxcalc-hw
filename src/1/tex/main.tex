...
\salign[*]{(2 - \varepsilon)\alpha \approx 2\alpha, \quad 2\alpha \approx \ln{\frac{1}{1 - \varepsilon}}, \quad \varepsilon = 1 - e^{-2\alpha},}
...
% \salign{x_0 = 1 + \varepsilon, \quad 0 < \varepsilon \ll 1.}
% От полученного данной подстановкой уравнения $\ln{(1 + \varepsilon)} = e^{-\alpha(1 + \varepsilon)}$ возьмем экспоненту:\
% \salign[*]{1 + \varepsilon = e^{e^{-\alpha(1 + \varepsilon)}},}
% и, так как $\xi = e^{-\alpha(1+\varepsilon)} \ll 1$, разложим правую часть по степеням $\xi$:
% \salign[*]{e^\xi \approx 1 + \xi,}
% откуда получаем $\varepsilon \approx \xi \approx e^{-\alpha(1+\varepsilon)}$, и, подставляя $\varepsilon$ в (1), получаем ответ:
% \salign[*]{\boxed{x_0 \approx 1 + e^{-\alpha}}}
